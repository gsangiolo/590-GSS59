...
# Take Weights and Biases vector and return submatrices
def extract_submatrices(weights_biases):
    submatrices = []; k=0
    for i in range(len(layers) - 1):
        n_row = layers[i+1]; n_col = layers[i]
        weights = np.array(weights_biases[k:k+n_row*n_col].reshape(n_col, n_row).T)
        k += n_row * n_col
        n_col = 1
        biases = np.transpose(np.array([weights_biases[k:k+n_row*n_col]]))
        k += n_row*n_col
        submatrices.append(weights.T)
        submatrices.append(biases.T)
    return submatrices

layers[0] = X.shape[1]
NFIT = 0 # Initialize the number of fitting parameters
for i in range(len(layers) - 1):
        NFIT = NFIT + layers[i] * layers[i+1] + layers[i+1]
print("Number of fitting parameters: " + str(NFIT))
...
def model(x,p):
        if(model_type=="linear" or model_type=="logistic" ):   
                ...
                out = x
                for i in range(len(submatrices)): # submatrices alternate weights -> biases -> weights...
                        if (i % 2) == 0:
...
def loss(p,index_2_use):
        ...
        if (GAMMA_L1 != 0.0):
            training_loss += GAMMA_L1 * np.sum(np.absolute(p))
        elif (GAMMA_L2 != 0.0): # Exclusive or do these go together sometimes?
            training_loss += GAMMA_L2 * np.sum(p**2.0)
