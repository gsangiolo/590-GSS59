...
# Flags for Settings
# ------------------------------------
dataset = 'CIFAR-10' # MNIST, FASHION-MNIST, or CIFAR-10
n_classes = 10 # 10 for MNIST, FASHION-MNIST, and CIFAR-10
method = 'DFF' # CNN, DFF, or 'LOAD path/to/model' (Convolutional Neural Network or Dense Feed Forward or Load a saved model)
...
if dataset == 'MNIST':
    ...
    (test_images, test_labels), (test_val_images, test_val_labels) = cifar100.load_data()
else:
...
if 'LOAD' not in method:
    model.add(layers.Dense(np.prod(image_dim), activation='sigmoid'))
...
if IS_AUGMENTED:
    ...
    iterator = datagen.flow(train_images, train_images.reshape(train_images.shape[0], np.prod(train_images.shape[1:])))
    history = model.fit_generator(iterator, steps_per_epoch=floor(train_total / N_NODES), validation_data=(val_images, val_images.reshape(val_images.shape[0], np.prod(val_images.shape[1:]))))
#model.compile(optimizer='rmsprop',
#              loss='categorical_crossentropy',
#              metrics=['accuracy'])
else:
    history = model.fit(train_images, train_images.reshape(train_images.shape[0], np.prod(train_images.shape[1:])), epochs=epochs, batch_size=batch_size, validation_data=(val_images, val_images.reshape(val_images.shape[0], np.prod(val_images.shape[1:]))))


#-------------------------------------
#EVALUATE ON TEST DATA
#-------------------------------------
train_loss, train_acc = model.evaluate(train_images, train_images.reshape(train_images.shape[0], np.prod(train_images.shape[1:])), batch_size=batch_size)
val_loss, val_acc = model.evaluate(val_images, val_images.reshape(val_images.shape[0], np.prod(val_images.shape[1:])), batch_size=floor(validation_percentage*batch_size))
test_loss, test_acc = model.evaluate(test_images, test_images.reshape(test_images.shape[0], np.prod(test_images.shape[1:])), batch_size=test_images.shape[0])
