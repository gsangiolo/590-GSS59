...
save='my_model' # None if no save, path/to/model to save
L2_CONSTANT = 1e-4
N_NODES = 64
ACT_TYPE = 'relu'
N_HIDDEN = 4
...
if IS_AUGMENTED:
    ...
    iterator = datagen.flow(train_images, train_labels)
    history = model.fit_generator(iterator, steps_per_epoch=floor(train_total / N_NODES), validation_data=(val_images, val_labels))
#model.compile(optimizer='rmsprop',
#              loss='categorical_crossentropy',
#              metrics=['accuracy'])
else:
