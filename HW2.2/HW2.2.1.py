...
for x1 in np.linspace(-5,5,N):
	...
	y=2.718*10*x1+3.14*x1+100.0+noise
...
#------------------------
def model(x,p):
    linear = np.matmul(x, p[:-1].reshape(NFIT-1, 1))
    if(model_type=="linear"):   return  linear+p[-1]  
    if(model_type=="logistic"): return  1.0/(1.0+np.exp(-(linear)))
